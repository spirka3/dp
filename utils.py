...
def read_label(label, img_width=W_DIMENSION, img_height=H_DIMENSION):
    # read label and return x, y, w, h
    c, x, y, w, h = label.split(' ')
    w = float(w) * img_width
    h = float(h) * img_height
    x = float(x) * img_width - w / 2
    y = float(y) * img_height - h / 2
    return c, int(x), int(y), int(w), int(h)


def write_label(label, img_width=W_DIMENSION, img_height=H_DIMENSION):
    c, x, y, w, h = label
    w = w / img_width
    h = h / img_height
    x = x / img_width + w / 2
...
def heatmap_visualizer(iou_results, dst_path):
...
def test_labels(img_src, label_src, output_src='./test.jpg'):
    img = cv2.imread(img_src)
    with open(label_src, 'r') as f:
        lines = f.readlines()
        for line in lines:
            label = read_label(line, img.shape[1], img.shape[0])
            draw_label(img, label, detected=False)
    cv2.imwrite(output_src, img)


def draw_label(img, label, detected=True, color=(12, 25, 255), border=2, v=5):
    if detected:
        c, x1, y1, w, h, conf = label
    else:
        c, x1, y1, w, h = label
        conf = ''
    if v == 5:
    ...
    draw_text(img, f'{c} {conf}', pos=(x1 - border, y1), text_color_bg=color, padding=border)
